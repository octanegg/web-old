export const calcPlayersOverview = (gamesData) => {
  const playersOverview = { blue: {}, orange: {} }
  const bluePlayers = []
  const orangePlayers = []

  // Fill allPlayers with values - in case there were substitutions
  gamesData.forEach((gamePlayers) =>
    gamePlayers.blue.forEach(
      (player) => !bluePlayers.some((_) => _ === player.name) && bluePlayers.push(player.name)
    )
  )
  gamesData.forEach((gamePlayers) =>
    gamePlayers.orange.forEach(
      (player) => !orangePlayers.some((_) => _ === player.name) && orangePlayers.push(player.name)
    )
  )

  bluePlayers.forEach((playerName) =>
    sumPlayerStats(playerName, 'blue', gamesData, playersOverview)
  )
  orangePlayers.forEach((playerName) =>
    sumPlayerStats(playerName, 'orange', gamesData, playersOverview)
  )
...
const sumPlayerStats = (playerName, playerTeam, rawData, outputObject) => {
  ...
  const currentPlayerGames = rawData.filter((game) =>
    game[playerTeam].some((p) => p.name === playerName)
  )

  if (currentPlayerGames.length === 0) return

  // Sum all the player's stats
  const summedStats = currentPlayerGames
    .map((game) => game[playerTeam].find((player) => player.name === playerName)?.stats)
    .reduce((all, current) => sumObjectValues(all, current), {})

  // Calculate averages for each stat, of current player.
  Object.keys(summedStats).forEach(
    (stat) => (summedStats[stat] = summedStats[stat] / currentPlayerGames.length)
  )
