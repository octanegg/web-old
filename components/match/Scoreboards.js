...
const GameDataPanel = ({ blueTeam, orangeTeam, blueTeamScore, orangeTeamScore, isOverview, data }) => <Flex width="100%" flexDirection="column">
    ...
    <TeamStatsTable columns={DEFAULT_ORDER} isOverview={isOverview} data={isOverview ? data?.blue : data.blue.players.reduce((all, current) => { all[current.name] = current.stats; return all; }, {})} />
    <TeamBlock teamName={orangeTeam} teamScore={orangeTeamScore} winner={blueTeamScore < orangeTeamScore} />
    <TeamStatsTable columns={DEFAULT_ORDER} isOverview={isOverview} data={isOverview ? data?.orange : data.orange.players.reduce((all, current) => { all[current.name] = current.stats; return all; }, {})} />
</Flex>;
...
const Scoreboards = (props) => {
    ...
    return <Flex margin="2rem 0" width="100%" flexDirection="column" border="1px solid #ddd" shadow="0 3px 5px rgba(0,0,0,.15)">
